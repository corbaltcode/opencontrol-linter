  ...
  context 'when checking a correct file' do
    it 'returns 0 for valid components file' do
      specifications = [{
        action: :run,
        targets: {
          components: ['./spec/fixtures/no_issues/components/AU_policy/component.yaml']
        }
      }]
    ...
    it 'returns 0 for valid standards file' do
      specifications = [{
        action: :run,
        targets: {
          standards: ['./spec/fixtures/no_issues/standards/FRIST-800-53.yaml']
        }
      }]
      specifications.each do |specification|
        expect(Opencontrol::Linter.run(specification)).to eq(0)
    ...
    it 'returns 0 for valid components file' do
      specifications = [{
        action: :run,
        targets: {
          certifications: ['./spec/fixtures/no_issues/certifications/FredRAMP-low.yaml']
        }
      }]
      specifications.each do |specification|
        expect(Opencontrol::Linter.run(specification)).to eq(0)
    ...
    it 'returns 0 for valid opencontrol file' do
      specifications = [{
        action: :run,
        targets: {
          opencontrols: ['./spec/fixtures/no_issues/opencontrol.yaml']
        }
      }]
      specifications.each do |specification|
        expect(Opencontrol::Linter.run(specification)).to eq(0)
  ...
  context 'when checking an incorrect file' do
    it 'checks a given file with faults and returns 1 for components file' do
      specification = {
        action: :run,
        targets: {
          components: ['./spec/fixtures/issues/components/AU_policy/component.yaml']
        }
      }
    ...
    it 'checks a given file with faults and returns 1 for standards file' do
      specification = {
        action: :run,
        targets: {
          standards: ['./spec/fixtures/issues/standards/FRIST-800-53.yaml']
        }
      }
      expect(Opencontrol::Linter.run(specification)).to eq(1)
      expect do
    ...
    it 'checks a given file with faults and returns 1 for certifications' do
      specification = {
        action: :run,
        targets: {
          certifications: ['./spec/fixtures/issues/certifications/FredRAMP-low.yaml']
        }
      }
      expect(Opencontrol::Linter.run(specification)).to eq(1)
      expect do
    ...
    it 'checks a given file with faults and returns 1 for opencontrol file' do
      specification = {
        action: :run,
        targets: {
          opencontrols: ['./spec/fixtures/issues/opencontrol.yaml']
        }
      }
      expect(Opencontrol::Linter.run(specification)).to eq(1)
      expect do
  ...
  context 'file checks on input' do
    it 'should emit a warning when no file is found to validate' do
      specification = {
        action: :run,
        targets: {
          components: ['./spec/fixtures/empty/**/component.yaml']
        }
      }
      expect(Opencontrol::Linter.run(specification)).to eq(1)
      expect do
    ...
    it 'should emit a warning when bad yaml is supplied\
      ...
      specification = {
        action: :run,
        targets: {
          components: ['./spec/fixtures/bad_yaml/component_of_bad.yaml']
        }
      }
